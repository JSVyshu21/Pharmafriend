...
os.environ['FIREBASE_JSON_PATH'] = 'SS.json'
os.environ['FIREBASE_API_KEY'] = 'AIzaSyCLwJjU11_O5LkJo1vXEL4U0dO6JfoPQaI'
...
st.set_page_config(
    page_title="PharmaFriend .ai",
    page_icon=r"static\\h3.png",
    layout="wide",
...
def generate_report(name, age, disease, description, precautions, workouts, diets, medications, file_path):
    ...
    title = Paragraph("PharmaFriend  Health Report", styleH)
...
def account():
    st.image(r"static\\h3.png")
    st.title("Welcome to PharmaFriend  🩺")
...
with st.sidebar:
    selected = option_menu(
        menu_title="PharmaFriend .ai",
        options=["Home", "Hello Doctor", "Doctor Report", "Some Home medications", "About", "Myself"],
...
if selected == "Home":
    ...
    with col1:
        st.title('PharmaFriend (Virtual Home Doctor) 🩺')
        st.header("Your Personalized 🪄 Doctor Buddy 👨🏻‍⚕️")
        st.divider()
        st.header("About 👨🏻‍⚕️🩺")
        st.markdown('''
        PharmaFriend.ai is your ultimate companion in health management, transforming how you care for yourself. 
                    With cutting-edge machine learning technology, PharmaFriend.aioffers precise predictions of 
                    potential diseases based on your symptoms. This empowers you with valuable insights and timely 
                    guidance, enabling you to take proactive control of your health journey.
        ''')
        st.markdown('''
        ### Join the `PharmaFriend `  😃
        PharmaFriend.ai is your ultimate companion in health management, transforming how you care for yourself. 
                    With cutting-edge machine learning technology, PharmaFriend.aioffers precise predictions of 
                    potential diseases based on your symptoms. This empowers you with valuable insights and timely 
                    guidance, enabling you to take proactive control of your health journey.
        ''')
        st.markdown("_Stay healthy, stay informed, and let PharmaFriend  be your trusted health companion_ 💪❤️")

        st.markdown("#### Get Started Now!")
        st.markdown("PharmaFriend  is here to help you live your healthiest life!")
    ...
    col1, col2 = st.columns([2, 1])
    with col1:
        st.title('PharmaFriend  WorkFlow ⛑️')
        ...
        st.markdown('''
            * Simply enter the symptoms you're experiencing into our user-friendly interface. Whether it's a 
            headache, fever, or any other discomfort, PharmaFriend .ai is here to help.
        ''')
        ...
        st.markdown('''
            * Once a disease is predicted, PharmaFriend .ai provides a comprehensive description of the condition. You'll learn about its causes, symptoms, and potential treatments, helping you understand your health better.
        ''')
        st.subheader("4️⃣ Personalized Recommendations 🌿💊")
        st.markdown('''
            PharmaFriend  goes beyond mere diagnosis. It offers personalized recommendations for:
    ...
    with col2:
        st.image(r"static\\h4.png")


elif selected == "Doctor Report":
    col1, col2 = st.columns([2, 1])
    with col1:
        if st.session_state.get("signedOut", False):
            st.title(f"Welcome {st.session_state.user_name} 🎉")
            st.header("PharmaFriend  Medical Report Generation 📃")
            st.divider()
            ...
            with col4:
                age = st.number_input("Enter the patient Age below", placeholder="Age", value=None, min_value=1,
                                      max_value=100)
            generate = st.button("Generate PharmaFriend  Report ✨")
            st.warning("⚠️ This is an automated AI generated report prepared by PharmaFriend")
                ...
                if st.session_state.predicted:
                    if name and age:
                        generate_report(
                            ...
                            file_path=f"PharmaFriend _{name.title()}_Report.pdf"
                        )
                        with open(f"PharmaFriend _{name.title()}_Report.pdf", "rb") as file:
                            st.download_button(
                                label="Download Generated Report ✅",
                                data=file,
                                file_name=f"PharmaFriend _{name.title()}_Report.pdf",
                                mime="pdf",
                            )
                    else:
                        st.warning("⚠️ Please enter correct Name/Age to proceed")
                else:
                    st.warning("⚠️ It seems like you haven't got your PharmaFriend  Recommendations!")
...
elif selected == "Some Home medications":
    
    st.markdown('<iframe src="http://127.0.0.1:5000/" width="100%" height="600px"></iframe>', unsafe_allow_html=True)
